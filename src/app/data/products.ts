import { Product } from "../types";

export const products: Product[] = [
  {
    id: "cap-1",
    name: "Cap",
    price: 7000,
    category: "snapback",
    colors: ["Black", "Blue Green", "Blue", "Red", "Sky Blue"],
    images: [
      "/products/cap-black.jpeg",
      "/products/cap-blue-green.jpeg",
      "/products/cap-blue.jpeg",
      "/products/cap-red.jpeg",
      "/products/cap-sky-blue.jpeg",
      "/products/cap-blue.jfif",
    ],
    description: "Premium quality Vibe District cap. Available in multiple colors to match your style. Professional finish and comfortable fit.",
    stock: 50,
    isBestSeller: true,
  },
  {
    id: "snapback-1",
    name: "Snapback",
    price: 7000,
    category: "snapback",
    colors: ["Cream"],
    images: [
      "/products/snapback.jpeg",
    ],
    description: "Authentic Vibe District snapback. Premium corduroy finish with iconic LA embroidery. High-quality, affordable, and built for style.",
    stock: 25,
  },
  {
    id: "durag-1",
    name: "Premium Satin Durag",
    price: 3500,
    category: "durag",
    colors: ["Black", "Navy", "Burgundy", "Gold"],
    images: [
      "/products/durag.jpeg",
    ],
    description: "Ultra-smooth satin durag for perfect waves. Breathable and comfortable professional grade material.",
    stock: 100,
    isBestSeller: true,
  },
  {
    id: "beanie-1",
    name: "Vibe District Beanie",
    price: 7000,
    category: "beanie",
    colors: ["Army", "Fashion", "Grey Cross", "Lego"],
    images: [
      "/products/beanie-army-01.jpeg",
      "/products/beanie-fashion.jpeg",
      "/products/beanie-flor.jpeg",
      "/products/beanie-grey-cross.jpeg",
      "/products/beanie-lego-fashion.jpeg",
      "/products/beanie-inspiration.jpeg", // 6-pack image joining the beanie collection
    ],
    description: "Keep warm and stylish with our premium beanies. High-quality knit with various designs. Premium, affordable, and high-quality.",
    stock: 40,
    isBestSeller: true,
  },
  {
    id: "finger-sleeves-1",
    name: "Gaming Finger Sleeves",
    price: 2000,
    category: "finger-sleeves",
    colors: ["Standard"],
    images: [
      "/products/finger-sleeves.jpeg",
      "/products/finger-sleeves-02.jpeg",
    ],
    description: "High-sensitivity gaming finger sleeves. Perfect for mobile gaming, providing smooth movement and sweat resistance. Sold per pair.",
    stock: 200,
  },
  {
    id: "skull-cap-1",
    name: "Skull Cap",
    price: 8000,
    category: "skull-cap",
    colors: ["Standard", "Red Signature"],
    images: [
      "/products/skull-cap-standard.jpeg",
      "/products/skull-cap-red-signature.jpeg",
    ],
    description: "Premium Skull Cap with premium finish. Perfect for under-helmet wear or as a minimal head covering. Available in Standard and Red Signature editions.",
    stock: 30,
    isNew: true,
  },
  {
    id: "cadet-cap-1",
    name: "Cadet Cap",
    price: 5000,
    category: "cadet-cap",
    colors: ["Army", "Black Fashioned"],
    images: [
      "/products/cadet-cap-army.jpeg",
      "/products/cadet-cap-black.jpeg",
    ],
    description: "Premium Cadet Cap with high-quality finish. Available in Army and Black Fashioned styles. Professional, stylish, and comfortable.",
    stock: 25,
    isNew: true,
  },
  {
    id: "necklace-1",
    name: "Dragon Eye Amulet Necklace",
    price: 5500,
    category: "necklace",
    colors: ["Silver"],
    images: ["/products/dragon-eye-necklace.jpeg"],
    description: "Intricately designed Dragon Eye Amulet. A mystical and bold statement piece for your collection. High-quality craftsmanship and durable finish.",
    stock: 15,
    isNew: true,
  },
  {
    id: "necklace-2",
    name: "Eagle Pendant Necklace",
    price: 5500,
    category: "necklace",
    colors: ["Gold/Silver"],
    images: ["/products/eagle-pendant-necklace.jpeg"],
    description: "Majestic Eagle Pendant Necklace. Symbolizing strength and freedom. Crafted with premium materials for a professional look.",
    stock: 20,
    isNew: true,
  },
  {
    id: "necklace-3",
    name: "Hip Hop Mystical Eye Necklace",
    price: 6500,
    category: "necklace",
    colors: ["Gold"],
    images: ["/products/mystical-eye-necklace.jpeg"],
    description: "Bold Hip Hop Mystical Eye Necklace. A standout piece that combines urban style with mystical elements. Premium quality and finish.",
    stock: 10,
    isNew: true,
  },
  {
    id: "pants-chain-1",
    name: "Spider Web Multi-Layer Pants Chain",
    price: 5500,
    category: "accessories",
    colors: ["Silver"],
    images: ["/products/spider-web-pants-chain.jpeg"],
    description: "Edgy Multi-Layer Pants Chain featuring a spider web design. Perfect for adding a unique touch to your outfit. Durable and stylish.",
    stock: 12,
    isNew: true,
  },
  {
    id: "custom-1",
    name: "Custom Design Cap",
    price: 15000,
    category: "custom",
    colors: ["Any Color"],
    images: [],
    description: "PREORDER: Create your own design! Add your name, logo, or custom embroidery. Hand-crafted specifically for you.",
    stock: 99,
    isPreorder: true,
  },
  {
    id: "watch-1",
    name: "Valenzo Luxury Wrist Watch",
    price: 7000,
    category: "watch",
    colors: ["Gold/Black"],
    images: ["/products/valenzo-watch.jpeg"],
    description: "The Valenzo Luxury Wrist Watch is a masterpiece of design and precision. Featuring a sleek gold finish with a deep black dial, it's the perfect statement piece for any outfit. High-quality craftsmanship for the bold and stylish.",
    stock: 10,
    isBestSeller: true,
  },
];
